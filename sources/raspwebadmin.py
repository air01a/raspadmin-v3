...
class HttpRequestHandler(BaseHTTPRequestHandler):
	...
	def gethttpvar(self,path):
		...
		split_url = urllib.parse.urlsplit(path)
		http_get = urllib.parse.parse_qs(split_url[3])
		for i in list(http_get.keys()):
			http_get[i]=http_get[i][0]

		cookies=None
		if "Cookie" in self.headers:
			cookies={}
			cookiesobj = http.cookies.SimpleCookie(self.headers["Cookie"])
			for cookiename,morsel in cookiesobj.items():
	...
	def renderer(self,title,http_response,extraheaders):
		...
		if hasattr(http_response,'headers'):
			for header in list(http_response.headers.keys()):
		...
		if http_response.template==None:
			if not http_response.hasVar('outputfile' ):
				self.wfile.write(http_response.content.encode())
			else:
				try:
			...
			if not 'includefile' in list(http_response.content.keys()):
	...
	# Normalisez path
		...
		if http_response.template != None and http_response.content!=None and 'API' in list(sessionmanager.get_vars(sessionid).keys()):
