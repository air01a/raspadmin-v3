...
class WebManager(WebStructure.WebAbstract):
	...
	def get_html(self,http_context):
		...
		if 'num_logout' in list(http_context.http_get.keys()):
		...
		if 'CONNECTED' in list(sessionvars.keys()):	
			if not 'alphanum_json' in list(http_context.http_get.keys()):
				return None
			else:
				connected=True
				error='No Error'
		...
		http_post_varlist=list(http_post.keys())
		...
		if 'alphanum_json' in list(http_context.http_get.keys()):
			template=None
