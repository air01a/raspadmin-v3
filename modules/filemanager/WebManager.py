...
class WebManager(WebStructure.WebAbstract):
	...
	def get_html(self,http_context):
		...
		if http_context.suburl=='download' and 'base64_file' in list(get.keys()):
			return self.file_download(get['base64_file'])
		
		if http_context.suburl=='upload' and 'file_upload' in list(post.keys()) and 'filename_upload' in list(post.keys()) and 'base64_path' in list(post.keys()):
		...
		if 'alphanum_action' in list(post.keys()):
			...
			if action in list(self._function.keys()):
				func=self._function[action]
				ptr=func['ptr']
				args=func['args']
				for item in args:
					if item not in list(post.keys()):
						return self._return(None,1001,'Missing argument')
				return ptr(post)


		b64path=""
		if 'base64_path' in list(http_context.http_get.keys()):
