...
class WebManager(WebStructure.WebAbstract):
	...
	def manage_action(self,http_context):
		...
		if 'alphanum_servicestatus' in list(post.keys()):
			ret=self._dlna.check_status()
			return (ret,'Service status')


		if 'alphanum_servicestart' in list(post.keys()):
			ret=self._dlna.start()
			return (ret,'Service start')

		if 'alphanum_servicestop' in list(post.keys()):
			ret=self._dlna.stop()
			return (ret,'Service stop')

		if 'alphanum_servicereload' in list(post.keys()):
	...
	def get_html(self,http_context):
		...
		if http_context.suburl=='modify' and 'base64_path' in list(post.keys()):
			(error,errorstr)=self.modify(post,status)

		if http_context.suburl=='delete' and 'base64_path' in list(post.keys()):
