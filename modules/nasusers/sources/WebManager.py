...
class WebManager(WebStructure.WebAbstract):
	...
	def modifyuser(self,http_context):
		...
		if 'alphanum_action' not in list(post.keys()):
			return (0,"No error","")
		
		action=post['alphanum_action']
		if action=='add':
			if 'alphanum_user' not in list(post.keys()) or 'str_password' not in list(post.keys()):
				return (3,"Missing parameters","")
			if 'str_comment' not in list(post.keys()):
			...
			if 'alphanum_name' not in list(post.keys()):
				return (3,"Missing parameters","")
			...
			if 'alphanum_name' not in list(post.keys()) or 'str_password' not in list(post.keys()):
                                return (3,"Missing parameters","")
